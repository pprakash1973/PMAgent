...
export async function POST(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { evidence, domainContext } = await assembleGenerationContext(id, "risk_register", {
    industry: (project as any).industry ?? null,
    description: project.description ?? null,
    customer: (project as any).customer ?? null,
  });

  const content = await generateArtifact("risk_register", projectContext, requirements, evidence, domainContext) as any;
