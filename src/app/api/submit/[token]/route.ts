...
export async function POST(
  ...
  const rlToken = rateLimit(`submit:${tokenHash}`, { limit: 10, windowMs: 10 * 60 * 1000 });
  if (rlToken) return NextResponse.json({ error: "Too many attempts. Try again later." }, { status: 429 });
  const rlIp = rateLimit(`submit-ip:${clientIp(req)}`, { limit: 30, windowMs: 10 * 60 * 1000 });
  if (rlIp) return NextResponse.json({ error: "Too many attempts. Try again later." }, { status: 429 });
