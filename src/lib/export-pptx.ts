...
function kpiStrip(slide: any, kpis: { label: string; value: string; sub?: string }[], y = 1.0) {
  const w = 12.73 / kpis.length;
  kpis.forEach((k, i) => {
    const x = 0.3 + i * w;
    slide.addShape(slide.addShapeDefinition?.() ?? {}, { x, y, w: w - 0.15, h: 1.5, fill: { color: WASH }, line: { color: MID_WASH, width: 0.75 } });
    slide.addText(k.value, { x, y: y + 0.1, w: w - 0.15, h: 0.8, fontSize: 28, bold: true, color: TEAL, align: "center", fontFace: "Aptos" });
    slide.addText(k.label, { x, y: y + 0.9, w: w - 0.15, h: 0.3, fontSize: 9, color: DARK_GRAY, align: "center", fontFace: "Aptos" });
    if (k.sub) slide.addText(k.sub, { x, y: y + 1.2, w: w - 0.15, h: 0.25, fontSize: 8, color: DARK_GRAY, align: "center", italic: true, fontFace: "Aptos" });
  });
}

// ── PMI STATUS REPORT (pmi-status) ───────────────────────────────────────────
// 7 slides per skill spec: Cover, Executive Summary, Milestone Tracker,
// Schedule & Budget, Accomplishments & Next Steps, Issues & Risks, Decisions
...
function buildStatusReport(pptx: any, content: any, projectName: string) {
  ...
  // Slide 1: Cover
  titleSlide(pptx, projectName, `Status Report — ${period}`, "UST Project Management Office");

  // Slide 2: Executive Summary — 4-dimension RAG scorecard
  const execSlide = contentSlide(pptx, "Executive Summary", projectName, page++);
  const overallRag = content.overallStatus ?? content.ragStatus ?? "green";
  const ragFillColor = ragFill(overallRag);
  // Overall RAG badge
  execSlide.addShape(pptx.ShapeType.roundRect, { x: 10.8, y: 0.9, w: 2.2, h: 0.75, fill: { color: ragFillColor }, rectRadius: 0.1 });
  execSlide.addText(ragLabel(overallRag), { x: 10.8, y: 0.9, w: 2.2, h: 0.75, fontSize: 13, bold: true, color: WHITE, align: "center", valign: "middle", fontFace: "Aptos" });
  // 4 dimension RAG
  const dims = [
    { label: "Schedule", status: content.scheduleRag ?? content.scheduleStatus ?? "green", reason: safeStr(content.scheduleReason ?? "") },
    { label: "Cost", status: content.costRag ?? content.costStatus ?? "green", reason: safeStr(content.costReason ?? "") },
  ...
  dims.forEach((d, i) => {
    const x = 0.3 + i * 2.7;
    execSlide.addShape(pptx.ShapeType.ellipse, { x: x + 0.7, y: 1.0, w: 1.0, h: 1.0, fill: { color: ragFill(d.status) } });
    execSlide.addText(d.label, { x, y: 2.1, w: 2.6, h: 0.35, fontSize: 10, bold: true, color: SOFT_BLACK, align: "center", fontFace: "Aptos" });
    execSlide.addText(d.reason.slice(0, 60), { x, y: 2.45, w: 2.6, h: 0.4, fontSize: 8.5, color: DARK_GRAY, align: "center", wrap: true, fontFace: "Aptos" });
  });
  // Summary text
  execSlide.addText(safeStr(content.executiveSummary ?? content.summary ?? ""), { x: 0.3, y: 3.0, w: 12.73, h: 1.0, fontSize: 12, color: SOFT_BLACK, wrap: true, fontFace: "Aptos" });

  // Slide 3: Milestone Tracker
  const ms = content.milestoneStatus ?? content.milestones ?? [];
  if (ms.length) {
    tableSlide(pptx, "Milestone Tracker", ["Milestone", "Planned Date", "Forecast Date", "Variance", "Status"],
      ms.slice(0, 12).map((m: any) => [
        safeStr(m.name ?? m.milestone),
        safeStr(m.plannedDate ?? m.dueDate ?? m.date),
  ...
  // Slide 4: Schedule & Budget Performance
  const s4 = contentSlide(pptx, "Schedule & Budget Performance", projectName, page++);
  kpiStrip(s4, [
    { label: "Schedule % Complete", value: safeStr(content.percentComplete ?? content.scheduleComplete ?? "—") + "%", sub: "vs plan" },
    { label: "SPI", value: safeStr(content.spi ?? "—"), sub: "Schedule Performance" },
    { label: "CPI", value: safeStr(content.cpi ?? "—"), sub: "Cost Performance" },
    { label: "Budget Spent", value: safeStr(content.budgetSpent ?? content.actualCost ?? "—"), sub: "of " + safeStr(content.totalBudget ?? "budget") },
  ], 1.0);
  const finNote = safeStr(content.financialStatus ?? content.budgetNarrative ?? "");
  if (finNote) s4.addText(finNote, { x: 0.3, y: 2.7, w: 12.73, h: 0.8, fontSize: 11, color: SOFT_BLACK, wrap: true, fontFace: "Aptos" });

  // Slide 5: Accomplishments & Next Steps
  const s5 = contentSlide(pptx, "Accomplishments & Next Steps", projectName, page++);
  const acc = (content.accomplishments ?? []).slice(0, 7).map((a: unknown) => ({ text: `✓  ${safeStr(a)}`, options: { bullet: false, fontSize: 11, color: "005a3c", paraSpaceAfter: 6, fontFace: "Aptos" } }));
  const plan = (content.nextWeekPlan ?? content.plannedActivities ?? []).slice(0, 7).map((a: unknown) => ({ text: `→  ${safeStr(a)}`, options: { bullet: false, fontSize: 11, color: TEAL, paraSpaceAfter: 6, fontFace: "Aptos" } }));
  s5.addText("Completed This Period", { x: 0.3, y: 0.85, w: 6.0, h: 0.4, fontSize: 11, bold: true, color: GREEN, fontFace: "Aptos" });
  if (acc.length) s5.addText(acc, { x: 0.3, y: 1.3, w: 6.0, h: 5.0 });
  s5.addText("Planned Next Period", { x: 6.7, y: 0.85, w: 6.33, h: 0.4, fontSize: 11, bold: true, color: TEAL, fontFace: "Aptos" });
  if (plan.length) s5.addText(plan, { x: 6.7, y: 1.3, w: 6.33, h: 5.0 });
  ...
  if (issues.length) {
    s6.addText("Active Issues", { x: 0.3, y: 0.85, w: 12.73, h: 0.35, fontSize: 11, bold: true, color: SOFT_BLACK, fontFace: "Aptos" });
    const issRows = issues.slice(0, 4).map((i: any) => [safeStr(i.description ?? i.issue), safeStr(i.owner ?? "—"), safeStr(i.dueDate ?? "—"), safeStr(i.status ?? "Open")]);
    const issTable = [
      ["Issue", "Owner", "Due", "Status"].map((h) => ({ text: h, options: { bold: true, color: WHITE, fill: { color: PETROL }, fontSize: 8.5, fontFace: "Aptos" } })),
      ...issRows.map((r: string[], ri: number) => r.map((c: string) => ({ text: c, options: { fontSize: 8.5, color: SOFT_BLACK, fill: { color: ri % 2 === 0 ? WHITE : WASH }, fontFace: "Aptos" } }))),
    ];
    s6.addTable(issTable, { x: 0.3, y: 1.25, w: 12.73, colW: [6, 2.2, 1.8, 1.73], border: { color: MID_WASH } });
  }
  if (risks.length) {
    const yOff = issues.length ? 3.2 : 0.9;
    s6.addText("Top Risks", { x: 0.3, y: yOff, w: 12.73, h: 0.35, fontSize: 11, bold: true, color: SOFT_BLACK, fontFace: "Aptos" });
    risks.slice(0, 4).forEach((r: any, i: number) => {
      const riskText = typeof r === "string" ? r : safeStr(r.description ?? r.statement);
      const level = safeStr(r.severity ?? r.level ?? "");
      const lvlColor = level.toLowerCase().includes("critical") ? RED : level.toLowerCase().includes("high") ? "FFC000" : level.toLowerCase().includes("medium") ? "FFD700" : GREEN;
      s6.addShape(pptx.ShapeType.roundRect, { x: 0.3 + i * 3.2, y: yOff + 0.45, w: 3.0, h: 1.5, fill: { color: WASH }, line: { color: lvlColor, width: 2 }, rectRadius: 0.08 });
      s6.addText(riskText.slice(0, 80), { x: 0.35 + i * 3.2, y: yOff + 0.5, w: 2.9, h: 1.4, fontSize: 9, color: SOFT_BLACK, wrap: true, fontFace: "Aptos" });
    });
  }
  ...
  if (!decisions.length) {
    ...
    decisions.slice(0, 5).forEach((d: any, i: number) => {
      ...
      s7.addText(`${i + 1}.`, { x: 0.3, y: 1.0 + i * 0.9, w: 0.4, h: 0.7, fontSize: 14, bold: true, color: TEAL, valign: "top", fontFace: "Aptos" });
      s7.addText(text, { x: 0.75, y: 1.0 + i * 0.9, w: 12.0, h: 0.7, fontSize: 12, color: SOFT_BLACK, wrap: true, fontFace: "Aptos" });
    });
  }
}

// ── PMI CHARTER DECK (pmi-charter) ────────────────────────────────────────────
// 10 slides: Cover, Exec Summary, Strategic Alignment, Scope, Objectives,
// Milestones, Stakeholders, Risks, Budget & Resources, Authorization

function buildCharter(pptx: any, content: any, projectName: string) {
  let page = 1;

  // Slide 1: Cover
  titleSlide(pptx, content.projectTitle ?? projectName, `Project Charter  v${safeStr(content.version ?? "1.0")}  |  ${safeStr(content.date ?? "")}`, "UST Project Management Office");

  // Slide 2: Executive Summary
  const s2 = contentSlide(pptx, "Executive Summary", projectName, page++);
  const cols3 = [
    { label: "Business Problem", text: safeStr(content.businessCase ?? content.projectDescription ?? "") },
    { label: "Solution Approach", text: (content.objectives ?? []).slice(0, 2).map(safeStr).join("\n") },
    { label: "Expected Outcome", text: (content.successCriteria ?? []).slice(0, 2).map((c: any) => safeStr(c?.criterion ?? c)).join("\n") },
  ];
  cols3.forEach((col, i) => {
    const x = 0.3 + i * 4.28;
    s2.addShape(pptx.ShapeType.rect, { x, y: 0.85, w: 4.1, h: 0.35, fill: { color: TEAL } });
    s2.addText(col.label, { x, y: 0.85, w: 4.1, h: 0.35, fontSize: 10, bold: true, color: WHITE, align: "center", valign: "middle", fontFace: "Aptos" });
    s2.addText(col.text.slice(0, 220), { x, y: 1.25, w: 4.1, h: 4.5, fontSize: 10.5, color: SOFT_BLACK, wrap: true, fontFace: "Aptos" });
  });
  // KPI callouts
  kpiStrip(s2, [
    { label: "Total Budget", value: safeStr(content.budget?.total ?? content.budget ?? "TBD") },
    { label: "Timeline", value: safeStr(content.milestones?.[content.milestones.length - 1]?.targetDate ?? "TBD") },
    { label: "Team Size", value: safeStr(content.stakeholders?.length ?? "TBD") + " stakeholders" },
  ], 5.6);

  // Slide 3: Scope — In / Out
  ...
  s3.addShape(pptx.ShapeType.rect, { x: 0.3, y: 0.85, w: 6.1, h: 0.35, fill: { color: GREEN } });
  s3.addText("IN SCOPE", { x: 0.3, y: 0.85, w: 6.1, h: 0.35, fontSize: 10, bold: true, color: WHITE, align: "center", valign: "middle", fontFace: "Aptos" });
  inScope.slice(0, 8).forEach((s: unknown, i: number) => {
    s3.addText(`✓  ${safeStr(s)}`, { x: 0.3, y: 1.3 + i * 0.52, w: 6.1, h: 0.45, fontSize: 11, color: SOFT_BLACK, fontFace: "Aptos" });
  });
  s3.addShape(pptx.ShapeType.rect, { x: 6.93, y: 0.85, w: 6.1, h: 0.35, fill: { color: RED } });
  s3.addText("OUT OF SCOPE", { x: 6.93, y: 0.85, w: 6.1, h: 0.35, fontSize: 10, bold: true, color: WHITE, align: "center", valign: "middle", fontFace: "Aptos" });
  outScope.slice(0, 8).forEach((s: unknown, i: number) => {
    s3.addText(`✗  ${safeStr(s)}`, { x: 6.93, y: 1.3 + i * 0.52, w: 6.1, h: 0.45, fontSize: 11, color: SOFT_BLACK, fontFace: "Aptos" });
  });
  ...
  objs.slice(0, 5).forEach((obj: unknown, i: number) => {
    const x = 0.3 + (i % 3) * 4.24;
    const y = 0.9 + Math.floor(i / 3) * 2.5;
    s4.addShape(pptx.ShapeType.roundRect, { x, y, w: 4.0, h: 2.2, fill: { color: WASH }, line: { color: MID_WASH, width: 0.75 }, rectRadius: 0.1 });
    s4.addShape(pptx.ShapeType.rect, { x, y, w: 4.0, h: 0.35, fill: { color: TEAL } });
    s4.addText(`Objective ${i + 1}`, { x, y, w: 4.0, h: 0.35, fontSize: 9, bold: true, color: WHITE, align: "center", valign: "middle", fontFace: "Aptos" });
    s4.addText(safeStr(obj).slice(0, 180), { x: x + 0.1, y: y + 0.4, w: 3.8, h: 1.75, fontSize: 10, color: SOFT_BLACK, wrap: true, fontFace: "Aptos" });
  });
  ...
  if (milestones.length) {
    ...
    // Horizontal timeline
    s5.addShape(pptx.ShapeType.rect, { x: 0.5, y: 3.5, w: 12.33, h: 0.08, fill: { color: TEAL_L } });
    milestones.slice(0, 6).forEach((m: any, i: number) => {
      const x = 0.5 + i * (12.33 / Math.max(milestones.length - 1, 1));
      s5.addShape(pptx.ShapeType.ellipse, { x: x - 0.18, y: 3.32, w: 0.36, h: 0.36, fill: { color: TEAL } });
      s5.addText(safeStr(m.name ?? m.milestone).slice(0, 30), { x: x - 1.2, y: i % 2 === 0 ? 2.0 : 4.1, w: 2.4, h: 0.7, fontSize: 8.5, color: SOFT_BLACK, align: "center", wrap: true, fontFace: "Aptos" });
      s5.addText(safeStr(m.targetDate ?? m.date ?? ""), { x: x - 1.2, y: i % 2 === 0 ? 2.75 : 4.85, w: 2.4, h: 0.35, fontSize: 8, color: DARK_GRAY, align: "center", fontFace: "Aptos" });
    });
  }

  // Slide 6: Stakeholder Overview
  const stk = content.stakeholders ?? [];
  if (stk.length) {
    tableSlide(pptx, "Stakeholder Overview", ["Name", "Role", "Power", "Interest", "Quadrant", "Engagement"],
      stk.slice(0, 10).map((s: any) => [
        safeStr(s.name), safeStr(s.role ?? s.title), safeStr(s.power), safeStr(s.interest),
        safeStr(s.quadrant ?? "—"), safeStr(s.currentEngagement ?? s.engagementLevel ?? "—"),
      ]), projectName, page++);
  }

  // Slide 7: Top Risks at Initiation
  ...
  if (risks.length) {
    risks.slice(0, 5).forEach((r: any, i: number) => {
      ...
      s7.addShape(pptx.ShapeType.roundRect, { x: 0.3, y: 0.9 + i * 1.05, w: 12.73, h: 0.9, fill: { color: WASH }, line: { color: lvlColor, width: 1.5 }, rectRadius: 0.08 });
      s7.addShape(pptx.ShapeType.roundRect, { x: 0.3, y: 0.9 + i * 1.05, w: 1.1, h: 0.9, fill: { color: lvlColor }, rectRadius: 0.06 });
      s7.addText(level.slice(0, 8), { x: 0.3, y: 0.9 + i * 1.05, w: 1.1, h: 0.9, fontSize: 8, bold: true, color: WHITE, align: "center", valign: "middle", fontFace: "Aptos" });
      s7.addText(rText.slice(0, 150), { x: 1.5, y: 0.95 + i * 1.05, w: 11.5, h: 0.8, fontSize: 10, color: SOFT_BLACK, wrap: true, valign: "middle", fontFace: "Aptos" });
    });
  }
  ...
  kpiStrip(s8, [
    { label: "Total Budget", value: safeStr(budget.total ?? "TBD"), sub: safeStr(budget.currency ?? "") },
    { label: "Contingency", value: safeStr(budget.contingencyReserve ?? "TBD") },
    { label: "Funding Source", value: safeStr(budget.fundingSource ?? "TBD") },
  ], 1.0);
  const assumptions = content.assumptions ?? [];
  if (assumptions.length) {
    s8.addText("Key Assumptions", { x: 0.3, y: 3.0, w: 12.73, h: 0.35, fontSize: 11, bold: true, color: TEAL, fontFace: "Aptos" });
    assumptions.slice(0, 4).forEach((a: unknown, i: number) => {
      s8.addText(`•  ${safeStr(a)}`, { x: 0.3, y: 3.45 + i * 0.5, w: 12.73, h: 0.45, fontSize: 10.5, color: SOFT_BLACK, fontFace: "Aptos" });
    });
  }
  ...
  sigs.slice(0, 4).forEach((sig: any, i: number) => {
    const x = 0.3 + i * 3.2;
    s9.addShape(pptx.ShapeType.rect, { x, y: 2.0, w: 2.9, h: 0.04, fill: { color: SOFT_BLACK } });
    s9.addText(safeStr(sig.name ?? ""), { x, y: 2.1, w: 2.9, h: 0.35, fontSize: 10, color: SOFT_BLACK, align: "center", fontFace: "Aptos" });
    s9.addText(safeStr(sig.role ?? ""), { x, y: 2.45, w: 2.9, h: 0.35, fontSize: 9, color: DARK_GRAY, align: "center", fontFace: "Aptos" });
    s9.addText("Date: _______________", { x, y: 3.1, w: 2.9, h: 0.3, fontSize: 8.5, color: DARK_GRAY, align: "center", fontFace: "Aptos" });
  });

  // Slide 10: Thank you / Next steps
  const last = pptx.addSlide();
  last.background = { color: PETROL };
  last.addText("Thank You", { x: 1, y: 2.0, w: 11.33, h: 1.2, fontSize: 40, bold: true, color: WHITE, align: "center", fontFace: "Aptos" });
